...
public class PointsGiver : MonoBehaviour
{
    ...
    public void GiveOutPoints()
    {
        if (dataManager == null)
        {
            Debug.LogError("Error! data manager not present in scene");

            return;
        }

        dataManager.AssignPoints(gameType.gameType.ToString());

        if (gameType.gameType != StartGame.GameType.Arena &&
            gameType.gameType != StartGame.GameType.Recruit &&
            gameType.gameType != StartGame.GameType.Maze &&
            gameType.gameType != StartGame.GameType.Cars &&
            gameType.gameType != StartGame.GameType.TowerDefense) Debug.LogError("Game type does not match level");
